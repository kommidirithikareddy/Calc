...
export default function Category() {
  ...
  return (
    <Layout>
      ...
      <style>{`
        .cat-hero { background: var(--bg-card); border-bottom: 1px solid var(--border); padding: 30px 40px; }
        .cat-hero-inner { max-width: 1160px; margin: 0 auto; display: flex; align-items: center; gap: 18px; }
        ...
        .cat-filters { background: var(--bg-card); border-bottom: 1px solid var(--border); padding: 14px 40px; display: flex; gap: 7px; flex-wrap: wrap; }
        ...
        @media (max-width: 900px) { .cat-hero { padding: 22px 24px; } .cat-hero-badge { display: none; } .cat-filters { padding: 12px 24px; } .cat-body { padding: 24px 24px 44px; } .calc-grid { grid-template-columns: repeat(2,1fr); } }
        @media (max-width: 560px) { .calc-grid { grid-template-columns: 1fr; } .subcat-sec-header { flex-wrap: wrap; } }
      `}</style>
    </Layout>
  )
}
